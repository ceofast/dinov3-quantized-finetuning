...
class DinoV3Linear(nn.Module):
    ...
    def forward(self, pixel_values):
        ...
        cls = last_hidden[:, 0]  # Use CLS token
...
def create_model(model_name: str, num_classes: int, freeze_backbone: bool = True, quantization_config=None):
    ...
    backbone = AutoModel.from_pretrained(
        ...
        torch_dtype=torch.float16 if quantization_config is not None else "auto"
    )
...
def setup_training(model, train_loader, args):
    ...
    if args.use_4bit and BITSANDBYTES_AVAILABLE:
        # Use 8-bit AdamW optimizer for better memory efficiency with quantized models
        try:
            ...
            # Fallback to regular AdamW if 8-bit version is not available
    ...
    # Disable mixed precision for quantized models as it can cause issues
...
def train_model(model, train_loader, val_loader, optimizer, scheduler, criterion, scaler, args, device):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for i, batch in enumerate(train_loader, start=1):
            ...
            # Use mixed precision only if scaler is enabled (not for quantized models)
            if scaler.is_enabled():
                ...
                # For quantized models, run without mixed precision
                logits = model(pixel_values)
                loss = criterion(logits, labels)
                loss.backward()
                optimizer.step()
            ...
            if global_step % args.eval_every_steps == 0:
                ...
                print(
                    ...
                    f"train_loss={running_loss / args.eval_every_steps:.4f} "
                ...
                if not args.no_tracking:
                    trackio.log({
                        "epoch": epoch,
                        "step": global_step,
                        "train_loss": running_loss / args.eval_every_steps,
...
def main():
    ...
    if quantization_config is None:
        ...
        # For quantized models, we need to be more careful with device placement
        device = next(model.parameters()).device  # Get the device where the model was placed
